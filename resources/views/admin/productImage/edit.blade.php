                ...
                <form class="">
                    <div class="box-body">
                        ...
                        <div class="form-group" id="form-product_id">
                            <label for="product_id">Địa điểm xuất phát</label>
                            <select class="form-control" id="product_id" name="product_id">
                                <option value="0">-- chọn --</option>
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" {{ ($product->id == $productImage->product_id) ? 'selected' : '' }}>{{ $product->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group" id="form-new_image">
                            ...
                            @if ($productImage->image)
                                <img src="{{ asset($productImage->image) }}" width="200" alt="">
                            @endif
                        </div>

                        <div class="form-group" id="form-url">
                            <label for="url">URL</label>
                            <input type="text" class="form-control" id="url" name="url" placeholder="URL" value="{{$productImage->url}}">
                        </div>

                        <div class="form-group" id="form-position">
                            <label for="position">Vị trí hiển thị</label>
                            <input type="number" class="form-control" id="position" name="position" min="0" placeholder="Vị trí hiển thị" value="{{$productImage->position}}">
                        </div>

                        <div class="checkbox form-group" id="form-is_active">
                            <label>
                                <input type="checkbox" name="is_active" id="is_active" {{ ($productImage->is_active == 1) ? 'checked' : '' }}> Trạng thái hiển thị
                            </label>
                        </div>
                    ...
                    <div class="box-footer">
                        <a class="btn btn-primary add-image" data-id="{{ $productImage->id }}">Update</a>
...
@section('my_script')
    ...
    <script>
        $('.add-image').click(function (e) {
            ...
            var model = '/admin/productImage/' + $(this).attr('data-id');
            var data;
            data = new FormData();
